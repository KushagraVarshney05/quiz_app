...
const EventQuestionsPage = () => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/v1/question/getquestions/${eventId}`
      );
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      await axios.patch(
        `http://localhost:5000/api/v1/question/${editingId}`,
        updatedData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const selectedEventName =
    events.find((e) => e._id === eventId)?.event_name || "";
                ...
                <li key={q._id} className="border p-4 rounded shadow">
                  {editingId === q._id ? (
                    <div className="space-y-2">
                      ...
                      />
                      {marksError && (
                        <p className="text-red-500">{marksError}</p>
                      )}
